...
> 批量解密 QQ 音乐加密音频文件（`.mflac`/`.mgg`）为标准格式（`.flac`/`.ogg`），并补充专辑元数据，一站式搞定。
...
| 脚本 | 作用 |
|------|------|
| `run_gui_simple.bat` | 启动 GUI（前台） |
| `start_gui_english.bat` | 启动 GUI（英文提示 + 前置检查） |
| `run_supplement.bat` | 补充专辑元数据 |
| `auto_decrypt.bat` | 全流程解密 |
| `check_env.bat` | 环境检查 |
...
qqmusic-decryptor/
├── src/                        # 源码目录
│   ├── main_cli.py             # CLI 解密引擎
│   ├── supplement_album_metadata.py  # 专辑元数据工具
│   ├── metadata_utils.py       # 元数据工具库
...
4. 写入临时文件，重命名为最终格式
5. 可选步骤：通过 QQ Music API 补充元数据（封面、发行年份）

无需单独下载 `frida-server.exe` — Frida Python 包自带内置 helper 进程。

---

## ❓ 常见问题

**问：需要单独下载 frida-server 吗？**  
不需要。Frida Python 包自带内置 helper 进程，开箱即用。

**问：没有 VIP 账号能用吗？**  
不能。解密依赖 QQ Music 内部的解密函数，需要有效的 VIP 会话。

**问：为什么用 Frida 而不是直接解析文件？**  
QQ 音乐的加密文件采用私有格式，只能由 QQ Music 进程本身解密。Frida 让我们能实时调用原生解密函数。

**问：能在 Linux/macOS 上运行吗？**  
...
- [strelitzia-reg/qqmusic-decryptor](https://github.com/strelitzia-reg/qqmusic-decryptor) — GUI 版本灵感来源
